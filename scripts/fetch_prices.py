...
DB_NAME = os.path.join(os.path.dirname(__file__), "../price_history.db")
# Node.js サーバーが読み込む JSON ファイルの出力先
JSON_OUTPUT = os.path.join(os.path.dirname(__file__), "../data/live_prices.json")
...
def setup_database():
    ...
    additional_columns = {
        "review_average": "REAL", "review_count": "INTEGER",
        "item_name": "TEXT", "item_url": "TEXT",
        "image_url": "TEXT", "shipping_fee": "INTEGER"
    }
    ...
    try:
        ...
        for result in results:
            ...
            if price_elem and title_elem and link_elem:
                ...
                return {
                    "price": float(price_str), "review_average": None,
                    "review_count": None, "item_name": title_elem.text.strip(),
                    "item_url": item_url, "image_url": image_url, "shipping_fee": None
                }
        return None
    ...
    try:
        ...
        if items:
            ...
            return {
                ...
                "image_url": image_url, "shipping_fee": shipping_fee
            }
        return None
    except Exception as e:
        print(f"  ❌ 楽天取得失敗: {e}")
        ...
        if info is not None:
            cursor.execute('''
                INSERT OR REPLACE INTO prices (
                    date, site, price, review_average, review_count,
                    item_name, item_url, image_url, shipping_fee
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
...
def get_graph_json():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT date, site, price, review_average, review_count,
               item_name, item_url, image_url, shipping_fee
    ...
    for row in rows:
        ...
        grouped_data[date_str][site] = {
            "price": price, "review_average": review_avg,
            "review_count": review_cnt, "item_name": name,
            "item_url": url, "image_url": img, "shipping_fee": shipping
        }
    return list(grouped_data.values())

def save_to_json(data):
    """Node.js サーバーが読み込める JSON ファイルとして保存する"""
    with open(JSON_OUTPUT, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    print(f"✅ {JSON_OUTPUT} に保存しました（Node.js サーバーが自動で読み込みます）")
